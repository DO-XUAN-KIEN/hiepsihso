...
//        }
    ...
    public static void MainAttack(Map map, MainObject ObjAtk, MainObject focus, int idxSkill, LvSkill temp, int type) throws IOException {
        ...
        if (ObjAtk.isPlayer() && focus.isPlayer() && focus.typepk == -1)// đồ sát
        {
            ...
            if(!(map.map_id == 61) || (map.map_id == 36)) ObjAtk.hieuchien +=100;
        }
...
//                    || (focus.isBoss() && focus.template.mob_id == 192 && map.zone_id == 3 && ObjAtk.level < 100);
        ...
        if (focus.hp <= 0) {
            ...
            if (focus != null && focus.template != null
                    && !focus.isMobDiBuon() && !focus.isPlayer()
...
//            if (focus != null && focus.template != null && focus.template.mob_id >= 93 && focus.template.mob_id <= 100) { // house chien truong
//                p.update_point_arena(1);
//              //  Manager.gI().chatKTGprocess("@Server : " + p.name + " đã đánh sập " + focus.template.name);
//               // ChienTruong.gI().update_house_die(focus.template.mob_id);
//                return;
